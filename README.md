...
A `Dockerfile.backend` and `Dockerfile.frontend` are included at the project root, along with a `docker-compose.yml` that brings up the database, API server, and React client.
...
- MySQL service will be exposed on port **3306**.
- Backend API available at http://localhost:8080
- Frontend served on http://localhost:3000

Environment variables are set in the compose file; change them there if you need custom credentials or a different database name.

You can also build individual services:
```bash
# backend only
docker build -f Dockerfile.backend -t hospital-backend .
# frontend only
docker build -f Dockerfile.frontend -t hospital-frontend .
```

### Cleaning up
```bash
docker compose down -v   # stop containers and remove volumes (clears MySQL data)
```
